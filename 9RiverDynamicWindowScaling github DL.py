...
@author: DougMaverick, IdeaIgnite.ai
...
if getattr(sys, 'frozen', False):
    # If bundled executable, use the path to the temporary directory
    base_path = sys._MEIPASS
else:
    # If running as a regular Python script, use the script's directory
    base_path = os.path.dirname(__file__)

image_path = os.path.join(base_path, "pngegg.png")
deck_image = Image.open(image_path)
...
starting_bet = 2
player_score = 0
larger_underlined_font = pygame.font.Font(None, 48)
larger_underlined_font.set_underline(True)
...
def display_player_score():
    ...
    draw_rectangle(50, 50, felt_color, 300, 50)
...
while running:
    ...
    text_message = 'PRESS "r" TO PLAY AGAIN OR "q" TO QUIT'
    text_image = larger_underlined_font.render(text_message, True, font_color)
    text_rect = text_image.get_rect(center=(screen_width // 2, int(450 / SCALE)))  # Center the text on the screen
    window.blit(text_image, text_rect.topleft)
    pygame.display.flip()
