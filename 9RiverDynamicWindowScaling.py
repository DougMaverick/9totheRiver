...
def ask_play_again() -> bool:
    ...
    display_text('Play another hand? Y/N', int(680 / SCALE), int(1100 / SCALE))
    while True:
...
def display_eligible(switched: bool) -> None:
    """
...
def deal_flop(bet: int, selected: int, switched: bool, board: List[int]) -> Tuple[int, int, bool]:
    """
...
def deal_turn(bet: int, board: List[int]) -> int:
    """
...
def deal_river(board: List[int]) -> None:
    """
...
def bonus(hand: List[int], switched: bool, board: List[int]) -> int:
    """
...
def who_wins(bet: int, board: List[int], hands: List[List[int]], selected: int) -> None:
    global player_score
    """
...
def play_game(bet: int, switched: bool, selected: int, board: List[int], hands: List[List[int]]) -> bool:
    """
    ...
    display_bet(bet)
    display_eligible(switched)
    display_text('Please select a hand', int(680 / SCALE), int(600 / SCALE))
    selected = make_selection()

    (bet, selected, switched) = deal_flop(bet, selected, switched, board)
    display_bet(bet)
    display_eligible(switched)
    time.sleep(0.5)

    bet = deal_turn(bet, board)
    display_bet(bet)
    time.sleep(0.5)

    deal_river(board)
    who_wins(bet, board, hands, selected)
    time.sleep(5)
...
# Initialize the variables before the main game loop
bet = starting_bet
switched = False
selected = 0
deck = treys.Deck()
hands = [deck.draw(2) for _ in range(number_of_hands)]
board = deck.draw(5)

# Main game loop
while True:
    play_again = play_game(bet, switched, selected, board, hands)
    if not play_again:
        break
